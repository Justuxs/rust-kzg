...
use kzg::{Fr, G1};
use crate::kzg_types::{ZFr as Scalar, ZG1 as G1Projective};
...
pub(crate) fn pippenger<P, I>(points: P, scalars: I) -> G1Projective
where
    ...
    I: Iterator<Item = Scalar>,
{
    ...
    let mut columns = (0..digits_count).rev().map(|digit_index| {
        ...
        for i in (0..(buckets_count - 1)).rev() {
            buckets_intermediate_sum = buckets_intermediate_sum.add(&buckets[i]);
    ...
    columns.fold(hi_column, |total, p| mul_by_pow_2(&total, w as u32).add(&p))
}
            ...
            scalars
                ...
                .for_each(|(&scalar, base)| {
                    if scalar == fr_one {
                        ...
                        // lower bits.
                        scalar.fr.divn(w_start as u32);

                        // We mod the remaining bits by the window size.
                        let scalar = scalar.fr.0[0] % (1 << c);
